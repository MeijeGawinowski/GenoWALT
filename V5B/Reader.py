...
class Reader:
    ...
    def __init__(self):
        ...
        self.choice = "sampling" # choice=file will take filteQTL to generate QTL data and choice="sampling" will generate loci and qtl randomly distributed in the genome
    ...
    def dataQTL(self):
        ...
        if self.choice == "sampling":
            ...
            Nb_allel = np.random.choice(list(range(2,6)),size=N_qtl,replace=True)
            data_qtl = np.column_stack([type_qtl,chrom_qtl.astype(np.object),pos_qtl,val,names,dominance,Nb_allel])
